...
class RolePermissionLayout extends Rows
{
    ...
    private function generatedPermissionFields(Collection $permissionsRaw): array
    {
        return $permissionsRaw
            ->map(function (Collection $permissions, $title) {
                return $this->makeCheckWheelGroup($permissions, $title);
            })
            ->flatten()
            ->toArray();
    }
     ...
     * @return Collection
     */
    private function makeCheckWheelGroup(Collection $permissions, string $title): Collection
    {
        return $permissions
            ->map(function (array $chunks) {
                return $this->makeCheckWheel(collect($chunks));
            })
            ->flatten()
            ->map(function (CheckWheel $checkbox, $key) use ($title) {
                return $key === 0
     ...
     * @return CheckWheel
     */
    private function makeCheckWheel(Collection $chunks): CheckWheel
    {
        return CheckWheel::make('permissions.'.base64_encode($chunks->get('slug')))
